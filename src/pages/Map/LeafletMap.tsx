...
export const AddressSearched = async (address: string) => {
  // Traduce una dirección a coordenadas
  try {
    const result = geocodeAddress(address); 
    console.log('Coordenadas de la dirección:', (await result).lat, (await result).lon);
  } catch (error) {
  ...
  useEffect(() => {
    import('leaflet').then((L) => {
      if (typeof window !== 'undefined') {
        ...
        //Crea una funcion que centre el mapa en la ubicacion que se le pasa y le haga zoom
